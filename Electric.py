...
# --- 💡 អនុគមន៍ទាញយកលេខទឹកភ្លើងថ្មីចុងក្រោយបង្អស់ មកធ្វើជាលេខចាស់ខែនេះ ---
def get_last_utility_readings(user_id):
    last_electric_new = 0.0
    last_water_new = 0.0
    if os.path.exists(LOG_FILE):
        try:
            df = pd.read_csv(LOG_FILE, dtype={'User ID': str})
            # ស្វែងរកទិន្នន័យដែលត្រូវនឹង ID របស់អតិថិជន
            user_logs = df[df['User ID'] == str(user_id)]
            if not user_logs.empty:
                # យកជួរទិន្នន័យចុងក្រោយបង្អស់ (Latest Row)
                latest_row = user_logs.iloc[-1]
                last_electric_new = float(latest_row.get('Electric New', 0.0))
                last_water_new = float(latest_row.get('Water New', 0.0))
        except Exception:
            pass
...
# បង្កើតតម្លៃ Default សម្រាប់លេខចាស់
default_elec_old = 0.0
default_water_old = 0.0
...
if id_user:
    ...
    if existing_name:
        ...
        # 💡 ទាញយកលេខចុងក្រោយពីរបាយការណ៍ចាស់ មកដាក់ចូលលេខចាស់ខែនេះ
        default_elec_old, default_water_old = get_last_utility_readings(id_user)
        if default_elec_old > 0 or default_water_old > 0:
            st.toast(f"🔄 បានទាញយកលេខចាស់ពីខែមុនដោយស្វ័យប្រវត្ត៖ ភ្លើង={default_elec_old} | ទឹក={default_water_old}")
    else:
...
# 💡 ដាក់តម្លៃ value=default_elec_old ដែលទាញចេញពីខែមុន
old_num_electric = st.number_input("លេខថាមពលចាស់ (ភ្លើង) =", value=default_elec_old, key="old_elec")
...
# 💡 ដាក់តម្លៃ value=default_water_old ដែលទាញចេញពីខែមុន
old_num_water = st.number_input("លេខនាឡិកាចាស់ (ទឹក) =", value=default_water_old, key="old_water")
